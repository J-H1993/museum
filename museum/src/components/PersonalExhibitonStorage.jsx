export const createPersonalExhibit = (exhibitName) => {
  ...
  if (savedExhibits[exhibitName]) {
    alert(
      "An exhibit already exists with this name, please choose a different name for your exhibit."
    );
    return false;
  }
...
export const handleSaveToExhibit = (artifact, myExhibitName, collection) => {
  ...
  const duplicateItemWarning = savedExhibits[myExhibitName].some(
    (item) => item.id === objectToSave.id
  );
...
export const deleteItemFromExhibit = (exhibitName, itemId) => {
  ...
  if (savedExhibits[exhibitName]) {
    const updatedExhibit = savedExhibits[exhibitName].filter(
      (item) => item.id !== itemId
    );
