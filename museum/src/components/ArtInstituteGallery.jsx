...
const ArtInstituteGallery = ({
  ...
  useEffect(() => {
    ...
    const fetchArtworks = async () => {
      try {
        ...
        console.error(
          "Error fetching artworks from the Art Institute Of Chicago collection:",
          error.message
        );
      } finally {
        setIsLoading(false);
    ...
    const fetchArtworksbyExhibition = async () => {
      try {
        ...
        console.error(
          "Error fetching artworks for the chosen exhibit:",
          error.message
        );
      } finally {
        setIsLoading(false);
  ...
  const paginatedArtwork = selectedExhibition
    ? museumObjects.slice((page - 1) * itemsPerPage, page * itemsPerPage)
    : museumObjects;
